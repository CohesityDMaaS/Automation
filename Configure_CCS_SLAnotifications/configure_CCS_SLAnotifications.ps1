...
# ./configure_CCS_SLAnotifications.ps1 -apiKey ****** -regionId us-east-1 -ruleName "SLA_Alerts" -emailAddresses "test@duh.com, blah@test.com" -violations All -source server.domain.com

# process commandline arguments
[CmdletBinding()]
param (
    [Parameter(Mandatory = $True)][string]$apiKey,  # apiKey
    [Parameter(Mandatory = $True)][string]$regionId,  # CCS SQL Source Region Id
    [Parameter(Mandatory = $True)][string]$ruleName,  # SLA Alert Notification Rule Name
    [Parameter(Mandatory = $True)][string]$source, # reference Registered Source to validate CCS Cluster
...
$outfileName = "log-configure_CCS_SLAnotifications-$dateString.txt"
