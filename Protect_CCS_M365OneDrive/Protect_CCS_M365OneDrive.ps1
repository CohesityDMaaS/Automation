...
param (
    ...
    [Parameter()][array]$users,  # optional names of OneDrive(s) to protect
    [Parameter()][string]$userList = '',  # optional textfile of OneDrive(s) to protect
...
# gather list of OneDrive(s) to protect
$usersToAdd = @()
...
if($usersToAdd.Count -eq 0){
    Write-Host "No OneDrivees specified" -ForegroundColor Yellow
    exit
}
...
if(!$usersNode){
    Write-Host "Source $sourceName is not configured for O365 OneDrive(s)" -ForegroundColor Yellow
    exit
}
