...
# ./Register_CCS_AWSsources.ps1 -apiKey #### -regionId us-east-1 -awsRegion us-east-2 -AWSid #### -roleARN "AWS_ARN"
...
param (
    ...
    [Parameter(Mandatory = $True)][string]$regionId,  # CCS region where AWS Account ID is to be Registered
...
$outfileName = "$PSScriptRoot\log-Register_CCS_AWSsources-$dateString.txt"
$finalOutput = "$PSScriptRoot\log-CCSAWSinfo-$dateString.txt"
...
foreach($AWSaccount in $AWStoAdd){
    ...
    Write-Host "`nPreparing CCS Registration of AWS ID: " $AWSaccount
    write-output "`n$dateTime    INFO    Preparing CCS Registration of AWS ID: " $AWSaccount | Out-File -FilePath $outfileName -Append
    ...
    if($AWSaccount){
        ...
        Write-Host "`nSTEP 1 - Registering AWS Account ID $AWSaccount in CCS...`n" 
        write-output "`n$dateTime    INFO    STEP 1 - Registering AWS Account ID $AWSaccount in CCS...`n" | Out-File -FilePath $outfileName -Append 

        # prepare body of REST API Call
        $bodyJson = $body | ConvertTo-Json 
        write-host "`nSTEP 1 CCS AWS Accout Registration API Payload: `n$bodyJson"  
        write-output "`n$dateTime    INFO    STEP 1 CCS AWS Accout Registration API Payload: `n$bodyJson" | Out-File -FilePath $outfileName -Append  
        $bodyJson = ConvertTo-Json -Compress -Depth 99 $body 

        # register CCS AWS Account - STEP 1
        ...
        write-output "`n$dateTime    INFO    Response from STEP 1 CCS AWS Accout Registration API: `n$response" | Out-File -FilePath $outfileName -Append
        ...
        write-host "`nStep 1 of CCS AWS Account Registration completed SUCCESSFULLY!" -ForegroundColor Green
        write-host "CFT Template Body: `n$cftJSON"
        write-host "CFT Template Location: `n$awsCFTjson"
        write-output "`n$dateTime    INFO    Step 1 of CCS AWS Account Registration completed SUCCESSFULLY!" | Out-File -FilePath $outfileName
        ...
        if($awsLogin -eq $true){
            ...
            foreach($awsARN in $ARNtoAdd){
                ...
                if($awsCreds){
                    ...
                    Write-Host "`nNo AWS Credentials initialized on this box! If you are experiencing AWS permissions errors, please reference the following section of the Cohesity CCS Guide: `nhttps://docs.cohesity.com/baas/data-protect/aws-account-requirements.htm?tocpath=Amazon%20Web%20Services%7C_____1#IAMUserPermissionstoExecuteCFT`n" -ForegroundColor Yellow 
                    write-output "`n$dateTime    WARN    No AWS Credentials initialized on this box! If you are experiencing AWS permissions errors, please reference the following section of the Cohesity CCS Guide: `nhttps://docs.cohesity.com/baas/data-protect/aws-account-requirements.htm?tocpath=Amazon%20Web%20Services%7C_____1#IAMUserPermissionstoExecuteCFT`n" | Out-File -FilePath $outfileName -Append
                }
            }
        }
                ...
                $cfnStack = New-CFNStack -StackName cohesity-ccs -TemplateBody "$cftJSON" -Capability "CAPABILITY_NAMED_IAM"
                ...
                $cftStatus = Get-CFNStack -StackName cohesity-ccs 
                $cftStatus = $cftStatus.StackStatus
                    while($cftStatus -ne "CREATE_COMPLETE"){
                        $cftStatus = Get-CFNStack -StackName cohesity-ccs
                        $cftStatus = $cftStatus.StackStatus
                        sleep 15
                        write-host "Cohesity-CCS AWS CFT Stack Deployment Status: $cftStatus" -ForegroundColor Yellow 
                        write-output "`n$dateTime    INFO    Cohesity-CCS AWS CFT Stack Deployment Status: $cftStatus" | Out-File -FilePath $outfileName -Append 
                        ...
                        if($cftStatus -eq "ROLLBACK_COMPLETE"){
                            ...
                            $stackEvents = Get-CFNStackEvent -StackName "cohesity-CCS" -Region $awsRegion
                            Write-Host "`n'cohesity-CCS' AWS CFT Stack Creation Events: `n$stackEvents`n" -ForegroundColor Yellow 
                            write-output "`n$dateTime    WARN    'cohesity-CCS' AWS CFT Stack Creation Events: `n$stackEvents`n" | Out-File -FilePath $outfileName -Append

                        }
                    }

                    write-host "Cohesity-CCS AWS CFT Stack Deployment Status: $cftStatus" -ForegroundColor Green 
                    write-output "`n$dateTime    INFO    Cohesity-CCS AWS CFT Stack Deployment Status: $cftStatus"| Out-File -FilePath $outfileName -Append 

                    # output CFT Creation Events
                    $stackEvents = Get-CFNStackEvent -StackName "cohesity-CCS" -Region $awsRegion
                    Write-Host "`n'cohesity-CCS' AWS CFT Stack Creation Events: `n$stackEvents`n" -ForegroundColor Green 
                    write-output "`n$dateTime    INFO    'cohesity-CCS' AWS CFT Stack Creation Events: `n$stackEvents`n" | Out-File -FilePath $outfileName -Append
    }
    else{
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if($cftStatus -eq "CREATE_COMPLETE"){
        Write-Host "`nSTEP 4 - Validating CCS can communicate to AWS Account ID $AWSaccount using CFT Template Roles...`n" 
        write-output "`n$dateTime    INFO    STEP 4 - Validating CCS can communicate to AWS Account ID $AWSaccount using CFT Template Roles...`n" | Out-File -FilePath $outfileName -Append 
        ...
        Write-host "Response from CCS CFT Deployment Validation API: `n$validation" -ForegroundColor Green
        write-output "`n$dateTime    INFO    Response from CCS CFT Deployment Validation API: `n$validation" | Out-File -FilePath $outfileName -Append
        }
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if(!$validation){
        Write-Host "`nSTEP 5 - Fetching AWS IAM ARN associated with Registration of AWS Account ID $AWSaccount in CCS...`n" 
        write-output "`n$dateTime    INFO    STEP 5 - Fetching AWS IAM ARN associated with Registration of AWS Account ID $AWSaccount in CCS...`n" | Out-File -FilePath $outfileName -Append 
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if($iam_role_arn){
        Write-Host "`nProcessing AWS IAM ARN associated with Registration of AWS Account ID $AWSaccount in CCS...`n" 
        Write-output "`n$dateTime    INFO    Processing AWS IAM ARN associated with Registration of AWS Account ID $AWSaccount in CCS...`n" | Out-File -FilePath $outfileName -Append
        ...
        Write-Host "`nSTEP 6 -Finalizing Registration of AWS Account ID $AWSaccount in CCS...`n" 
        Write-output "`n$dateTime    INFO    STEP 6 - Finalizing Registration of AWS Account ID $AWSaccount in CCS...`n" | Out-File -FilePath $outfileName -Append
