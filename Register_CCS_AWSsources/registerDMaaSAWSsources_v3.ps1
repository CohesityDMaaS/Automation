...
foreach($AWSaccount in $AWStoAdd){
    ...
    if($AWSaccount){
        ...
        $response = api post -mcmv2 "dms/tenants/regions/aws-cloud-source" $bodyJson -region $regionId
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if($cftStatus -eq "CREATE_COMPLETE"){
        ...
        $validation = api get -mcmv2 "dms/tenants/regions/aws-cloud-source-verify?tenantId=$tenantId&destinationRegionId=$regionId&awsAccountNumber=$AWSaccount" -region $regionId 
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if(!$validation){
        ...
        $fetch = api get -mcmv2 "dms/tenants/regions/aws-cloud-source?tenantId=$tenantId&destinationRegionId=$regionId&awsAccountNumber=$AWSaccount" -region $regionId
...
#---------------------------------------------------------------------------------------------------------------#
    ...
    if($iam_role_arn){
        ...
        $final = api post -mcmv2 "data-protect/sources/registrations" $bodyJson -region $regionId
