...
# Process the nonprod_accounts.csv file
Import-Csv "nonprod_accounts.csv" | ForEach-Object {
    $AWSaccount = $_.account_id
    $AWSname = $_.account_name
    $AWSregion= $_.region
    ...
    if($AWSaccount){
        ...
        info_log "STEP 1 DMaaS AWS Accout Registration API Payload: `n$awsPayload"  
        # $bodyJson = ConvertTo-Json -Compress -Depth 99 $body 

        # register DMaaS AWS Account - STEP 1
        $response = api post -mcm '/dms/tenants/regions/aws-cloud-source' $awsPayload 
